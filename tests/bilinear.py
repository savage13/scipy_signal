...
for order, w0, w1 in pars:
    ...
    fs = 2.0 # see signal.iirfilter() Normalized filter
    z,p,k = signal.buttap(order)
    W = [w0, w1]
    bw = W[1] - W[0]
    wo = np.sqrt(W[0] * W[1])
    z,p,k = signal.lp2bs_zpk(z, p, k, wo, bw)
    zpk = signal.bilinear_zpk(z,p,k,fs)
    tests.append({
        "name": f"{suite_name} n={order} [{w0}, {w1}]",
        "js": [
            f"let fs = 2.0 // not analog ",
            f"let {{z,p,k}} = buttap({order})",
            f"let w = [{w0}, {w1}]",
            f"let bw = w[1] - w[0]",
            f"let wo = Math.sqrt(w[1] * w[0]);",
            f"({{z, p, k}} = lp2bs_zpk(z, p, k, wo, bw));"
            f"({{z, p, k}} = bilinear_zpk(z, p, k, fs));"
        ],
        "expect": zpk,
        "assert": [ "z", "p", "k" ]
    })
